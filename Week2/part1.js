...
const capital_of_country = `SELECT capital FROM country WHERE country.Name = '${userInput}'`;

// List all the languages spoken in the region Y (Accept Y from user)
const language_of_region = `SELECT Language FROM countrylanguage INNER JOIN country ON country.Code = countrylanguage.CountryCode WHERE country.Region = '${userInput}'`;
...
const language_of_city = `SELECT COUNT(Name) AS CityLanguage FROM city INNER JOIN countrylanguage ON countrylanguage.CountryCode = city.Country_Code WHERE countrylanguage.Language = '${userInput}'`;
...
const official_language = `SELECT Name FROM country INNER JOIN countrylanguage ON countrylanguage.CountryCode = country.Code WHERE countrylanguage.Language = '${userInput}' AND country.Region = '${userInput2}' AND countrylanguage.isOfficial ='T'`;

// List all the continents with the number of languages spoken in each continent
const language_of_continents =
  'SELECT COUNT(countrylanguage.Language) as languages, country.continent FROM country INNER JOIN countrylanguage ON countrylanguage.CountryCode = country.Code GROUP BY country.continent';

switch (inputType) {
  case 'country':
    sendQuery(capital_of_country);
    break;
  case 'region':
    sendQuery(language_of_region);
    break;
  case 'language':
    sendQuery(language_of_city);
    break;
  case 'language_and_region':
    console.log('userInput =', userInput, 'userInput2 =', userInput2);
    sendQuery(official_language);
    break;
  case 'continents':
