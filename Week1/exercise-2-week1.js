...
const countriesPopulation = 'SELECT name FROM country where population > 8000000;';

const findingCountries = 'SELECT name FROM country where name LIKE "%land%";';

const findingCountriesAccordingToPopulation = 'SELECT name FROM city WHERE population BETWEEN 500000 and 1000000;';

const nameOfEuropeCountries = 'SELECT name FROM country WHERE Continent = "Europe";';

const countriesSortAccordingToSurfaceArea = 'SELECT name, SurfaceArea FROM country ORDER BY SurfaceArea DESC;';

const getCitiesNameInNetherlands = 'SELECT name FROM city WHERE countrycode = "NLD";';

const getRotterdamPopulation = 'SELECT name, population FROM city WHERE name = "Rotterdam";';

const TopCountriesBySurfaceArea = 'SELECT name, SurfaceArea FROM country ORDER BY SurfaceArea DESC LIMIT 10;';
...
const CountriesPopulation = () => {
  connection.query(findingCountriesAccordingToPopulation, (err, res) => {
    err ? console.log(err) :
    console.log(res, 'cities with population in between 500,000 and 1 million');
  });
};
...
const countriesSurfaceArea = () => {
  connection.query(countriesSortAccordingToSurfaceArea, (err, res) => {
    err ? console.log(err) :
    console.log(res, 'countries sorted in the descending order of their surface areas');
  });
};
...
const rotterdamPopulation = () => {
  connection.query(getRotterdamPopulation, (err, res) => {
    err ? console.log(err) :
    console.log(res, 'the population of Rotterdam');
  });
};
