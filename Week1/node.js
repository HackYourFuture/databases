...
const table_of_country =
  'CREATE TABLE IF NOT EXISTS country (name VARCHAR(50), continent VARCHAR(50), region VARCHAR(50), surface_area FLOAT, indep_year INT, population INT, life_expectancy FLOAT, GNP FLOAT, GNPold FLOAT, local_name VARCHAR(50), government_form VARCHAR(50), head_of_state VARCHAR(50),capital VARCHAR(50))';
...
var insert_countries = [
  'INSERT INTO country (name, continent, surface_area, population) VALUES' +
    "                  ('Angola', 'Africa', 1246700, 12878000)," +
    "                  ('Syria', 'Asia', 185180, 18269868),   " +
    "                  ('Tanzania', 'Africa', 883749, 57310019)," +
    "                  ('Netherlands','Europe', 800, 17035938)," +
    "                  ('Afghanistan', 'Asia',  652090, 35530081)," +
    "                  ('Romania','Europe', 238391, 19679306)," +
    "                  ('Switzerland','Europe', 41284, 8476005)," +
    "                  ('Sweden', 'Europe', 449964, 9910701),",
];
...
var insert_cities = [
  'INSERT INTO city (id, name, country_code, population) VALUES ' +
    "(1, 'Amsterdam', 'NLD', 821752)" +
    "(2, 'Rotterdam','NLD', 623652)" +
    "(3, 'Kabul','KAB', 4600000)" +
    "(4, 'Bucharest','BUC', 1836000)" +
    "(5, 'Stockholm','STO',  965232)" +
    "(6, 'Damascus', 'DAM',1711000)" +
    "(7, 'Dodoma', 'DOD',2084000)" +
    "(8, 'Bern', 'BER', 133115)" +
    "(9,'Arnhem','NLD',138020)" +
    "(10,'Breda','NLD',160398)" +
    "(11,'Huambo','HUA',163100)" +
    "(12,'Lobito','LOB',130000)" +
    "(13,'Benguela','BEN',128300)" +
    "(14,'Zaanstad', 'ZAA',135621)",
];
...
database.query(`SELECT name FROM country WHERE Name LIKE '%land%' `, (error, result) => {
  if (error) throw error;
...
database.query(`SELECT * FROM country ORDER BY surface_area DESC`, (error, result) => {
  if (error) throw error;
