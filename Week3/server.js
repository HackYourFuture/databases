...
app.get('/user/:id', function (req, res) {
  db.getAll(req.params.id).then(result => {
    res.send(result);
...
app.post('/create-user/:id', function (req, res) {
  ...
  db.createUser(id, firstName, lastName).then(result => {
    res.send(result);
...
app.post('/create-list/:id', function (req, res) {
  ...
  db.createList(UserId, category, prioritize).then(result => {
    res.send(result);
  }).catch(error => console.log(error.message));
});
...
app.post('/create-item/:id', function (req, res) {
  ...
  let prioritize = req.body.prioritize;
  db.createItem(listId, todo, description, prioritize).then(result => {
    res.send(result);
  }).catch(error => console.log(error.message));
});
...
app.post('/item-completed/:id', function (req, res) {
  let listId = req.params.id;
  db.setItemCompleted(listId).then(result => {
    res.send(result);
  }).catch(error => console.log(error.message));
});
...
app.put('/update-list/:id', function (req, res) {
  ...
  let category = req.body.category;
  let prioritize = req.body.prioritize;
  db.updateList(category, prioritize, listId).then(result => {
    res.send(result);
  }).catch(error => console.log(error.message));
});
...
app.put('/update-item/:id', function (req, res) {
  ...
  let description = req.body.description;
  let prioritize = req.body.prioritize;
  db.updateList(todo, description, prioritize, itemId).then(result => {
    res.send(result);
  }).catch(error => console.log(error.message));
});
