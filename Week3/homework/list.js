...
async function addList(req, res) {
  try {
    const list = [req.body.name, req.params.person];

    const sql = `INSERT INTO lists(name,person) VALUES (?, ?)`;
    await executeSqlQuery(sql, list);

    const sqlSelect = `SELECT * FROM lists WHERE name=? AND person= ?`;
...
async function getLists(req, res) {
  try {
    ...
    const sqlSelect = `SELECT * FROM lists WHERE person=?`;
    const lists = await executeSqlQuery(sqlSelect, person);
