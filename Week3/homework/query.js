...
const getItems = async id => {
  const query = `select items.id id, items.description item_description, items.status status, 
    lists.id lid, lists.description list_description, lists.reminder_date remind_at 
    from lists left join items on lists.id = items.id where items.uid=?`;
